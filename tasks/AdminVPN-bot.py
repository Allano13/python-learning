...
async def add_user_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        conn.commit()
        conn.close()
        await update.message.reply_text(f"Пользователь с ID {user_id} добавлен в базу данных.")
        ...
        await update.message.reply_text("Произошла ошибка при добавлении пользователя. Попробуйте снова.")
        # Показываем меню снова
        keyboard = [
            [KeyboardButton("Добавить пользователя"), KeyboardButton("Удалить пользователя")],
            [KeyboardButton("Список пользователей"), KeyboardButton("Выдать ключ и инструкцию")]
    ...
    try:
        result = subprocess.run([GENERATE_KEY_SCRIPT, user_id, user_name], capture_output=True, text=True, check=True)
        await update.message.reply_text(f"Ключ для пользователя с ID {user_id} успешно сгенерирован.")
    except subprocess.CalledProcessError as e:
        logger.error(f"Ошибка при генерации ключа для ID {user_id}: {e}, stderr: {e.stderr}")
        await update.message.reply_text(f"Ошибка: Не удалось сгенерировать ключ для пользователя с ID {user_id}.")
    except FileNotFoundError:
        logger.error(f"Скрипт {GENERATE_KEY_SCRIPT} не найден")
        await update.message.reply_text("Ошибка: Скрипт генерации ключа не найден на сервере.")
    except Exception as e:
...
async def delete_user_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.message.reply_text("Произошла ошибка при удалении пользователя. Попробуйте снова.")
...
async def issue_key_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_id != ADMIN_ID:
        await update.message.reply_text("Доступ запрещён. Обратитесь к администратору.")
        return ConversationHandler.END
    await update.message.reply_text("Введите ID пользователя (например, 999):", reply_markup=ReplyKeyboardRemove())
...
async def issue_key_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Проверяем, что ID состоит только из цифр
    if not re.match(r'^\d+$', user_id):
        await update.message.reply_text("ID должен состоять только из цифр (например, 999). Попробуйте снова:")
        return ISSUE_KEY_STATE

    # Показываем меню снова (вынесено в отдельную функцию для повторного использования)
    async def show_menu():
        keyboard = [
            [KeyboardButton("Добавить пользователя"), KeyboardButton("Удалить пользователя")],
            [KeyboardButton("Список пользователей"), KeyboardButton("Выдать ключ и инструкцию")]
    ...
    try:
        ...
        await update.message.reply_text("Произошла ошибка при проверке пользователя в базе данных.")
        await show_menu()
        return ConversationHandler.END
    ...
    try:
        result = subprocess.run([SEND_KEY_SCRIPT, user_id], capture_output=True, text=True, check=True)
        key_path = result.stdout.strip().split('\n')[-1]  # Последняя строка — путь к ключу
        ...
        await update.message.reply_text(f"Ошибка: Ключ для пользователя с ID {user_id} не найден.")
    except FileNotFoundError:
        logger.error(f"Скрипт {SEND_KEY_SCRIPT} не найден")
        await update.message.reply_text("Ошибка: Скрипт отправки ключа не найден на сервере.")
    except Exception as e:
...
def main() -> None:
    ...
    issue_key_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Text() & ~filters.Command() & filters.Regex('^Выдать ключ и инструкцию$'), issue_key_start)],
        states={
